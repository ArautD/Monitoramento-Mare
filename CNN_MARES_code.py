...
#from keras.preprocessing.image import ImageDataGenerator old
...
#Classe 1 = indeterminado
#Classe 2 = maré baixa
#Classe 3 = maré mediana
...
idClass_mare_baixa  = 0
idClass_mare_mediana = 1
idClass_mare_alta = 2
...
def recover_data(path_images,idClass):
...
X_0, y_0 = recover_data(imageDirectory_mare_baixa,idClass_mare_baixa, )
X_1, y_1 = recover_data(imageDirectory_mare_mediana,idClass_mare_mediana)
X_2, y_2 = recover_data(imageDirectory_mare_alta,idClass_mare_alta)

X = np.concatenate((X_0, X_1, X_2)).copy()
y = np.concatenate((y_0, y_1, y_2)).copy()

print('Quantidade total de amostras totais:', y.shape)
print(y)
...
bars = ('maré_baixa', 'maré_mediana', 'maré_alta')
...
#Selecionar os dados somente da classe de maré alta e maré baixa para realizar o aumento dos dados dessa classe.
indexes_train = np.where( (y_train==0) | (y_train == 1) | (y_train==2))  #Aqui escolhe-se as classes que deseja-se aumentar
X_train_selectedClasses = X_train[indexes_train].copy()
y_train_selectedClasses = y_train[indexes_train].copy()

#Selecionar os dados somente da classe de maré alta e maré baixa para realizar o aumento dos dados dessa classe.
indexes_val = np.where( (y_val==0) | (y_val == 1) | (y_val==2))  #Aqui escolhe-se as classes que deseja-se aumentar
X_val_selectedClasses = X_val[indexes_val].copy()
y_val_selectedClasses = y_val[indexes_val].copy()
...
#Data augmentation (para somente as classes selecionadas do dataset ) - Gera amostras de dados pela operação de flip vertical e horizontal
datagen = ImageDataGenerator(horizontal_flip = True,vertical_flip = True)

#-------------------------------------------------------------------------------

#Data aumentation sobre os dados de treino
train_datagen = datagen.flow(X_train_selectedClasses,y_train_selectedClasses, batch_size=1)
...
#Data aumentation sobre os dados de validação
val_datagen = datagen.flow(X_val_selectedClasses,y_val_selectedClasses, batch_size=1)
# generate samples and plot
#fig, ax = plt.subplots(nrows=1, ncols=len(train_datagen)*2, figsize=(100,100))

# generate batch of images
for i in range(len(val_datagen)*2):
  # convert to unsigned integers
  image_label = next(val_datagen)
  image = image_label[0][0]
  label = image_label[1][0]
  #print(image.shape)
  #print(image)
  ...
  #ax[i].imshow(image)
  #ax[i].axis('off')
  listImgGen_val.append(image)
  listLabelGen_val.append(label)
...
for i in range(len(train_datagen)*2):
  ...
  #ax[i].imshow(image)
  #ax[i].axis('off')
  listImgGen.append(image)
  listLabelGen.append(label)'''
...
#Concatenar dados de validação com dados gerados
numpyArrimgGen_val= np.array(listImgGen_val).copy()
X_val_gen = np.concatenate((X_val, numpyArrimgGen_val)).copy()

numpyArrLabelGen_val = np.array(listLabelGen_val).copy()
print('Quantidade de amostras dos dados de validação geradas', numpyArrLabelGen_val.shape)
y_val_gen = np.concatenate((y_val, numpyArrLabelGen_val)).copy()
print('Quantidade de dados de validação + amostras geradas', y_val_gen.shape)

#Transformando os rótulos de decimal para vetores com valores binários
y_train_gen = np_utils.to_categorical(y_train_gen).copy()
y_val_gen = np_utils.to_categorical(y_val_gen).copy()
y_test = np_utils.to_categorical(y_test).copy()
...
# loss='categorical_crossentropy' para multi classes
# loss='binary_crossentropy' para classes binárias
...
callback = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=3,mode="auto",verbose=1)

history = model.fit(X_train_gen, y_train_gen, epochs=epochs, batch_size=batch_size, validation_data=(X_val_gen, y_val_gen),callbacks=[callback])
